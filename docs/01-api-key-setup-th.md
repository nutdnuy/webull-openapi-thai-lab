...
เมื่อ configuration โหลดสำเร็จ คำสั่ง `doctor` จะแสดง Webull environment, region, trading endpoint, app key, app secret และ account id โดย redact ค่า sensitive ก่อนแสดงผล เช่น `abcd...wxyz` หรือ `<not set>` สำหรับ account id ที่ยังไม่ตั้งค่า

## Expected errors แบบอ่านง่าย

ถ้า `.env` ยังขาด `WEBULL_APP_KEY` หรือ `WEBULL_APP_SECRET`, `webull-lab doctor` จะโหลด configuration ไม่สำเร็จและอาจแสดง error จาก runtime โดยตรง. Error boundary แบบ concise มีไว้สำหรับคำสั่ง `webull-lab account-list`, `webull-lab stock-snapshot AAPL` และ `webull-lab preview-stock-buy AAPL 100 1` สำหรับ expected errors เช่น config ไม่ครบ, API response error หรือ input validation fail
